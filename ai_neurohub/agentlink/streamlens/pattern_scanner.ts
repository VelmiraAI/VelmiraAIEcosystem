...
export class CandlestickPatternDetector {
  ...
  /* Fetch recent OHLC candles with timeout via AbortController */
  async fetchCandles(symbol: string, limit = 100, timeoutMs = 10_000): Promise<Candle[]> {
  ...
  private isBullishEngulfing(prev: Candle, curr: Candle): number {
    ...
    return bodyPrev > 0 ? Math.min(bodyCurr / Math.max(bodyPrev, 1e-9), 1) : 0.8
  }
  ...
  /**
   * Detect patterns across a candle array
   * @param candles input candles (unsorted allowed)
   * @param minConfidence filters signals below threshold [0..1]
   */
  detectAll(candles: Candle[], minConfidence = 0.2): PatternSignal[] {
    ...
    for (let i = 0; i < series.length; i++) {
      ...
      if (hammer >= minConfidence) out.push({ timestamp: c.timestamp, pattern: "Hammer", confidence: this.clamp(hammer) })

      const star = this.isShootingStar(c)
      if (star >= minConfidence) out.push({ timestamp: c.timestamp, pattern: "ShootingStar", confidence: this.clamp(star) })

      if (prev) {
        const bull = this.isBullishEngulfing(prev, c)
        if (bull >= minConfidence) out.push({ timestamp: c.timestamp, pattern: "BullishEngulfing", confidence: this.clamp(bull) })

        const bear = this.isBearishEngulfing(prev, c)
        if (bear >= minConfidence) out.push({ timestamp: c.timestamp, pattern: "BearishEngulfing", confidence: this.clamp(bear) })
      }

      const doji = this.isDoji(c)
      if (doji >= minConfidence) out.push({ timestamp: c.timestamp, pattern: "Doji", confidence: this.clamp(doji) })
    }

    return this.deduplicateKeepMax(out)
  }

  /**
   * Detect patterns for the latest candle only (uses previous candle when needed)
   */
  detectLatest(candles: Candle[], minConfidence = 0.2): PatternSignal[] {
    ...
    if (hammer >= minConfidence) signals.push({ timestamp: last.timestamp, pattern: "Hammer", confidence: this.clamp(hammer) })

    const star = this.isShootingStar(last)
    if (star >= minConfidence) signals.push({ timestamp: last.timestamp, pattern: "ShootingStar", confidence: this.clamp(star) })

    if (prev) {
      const bull = this.isBullishEngulfing(prev, last)
      if (bull >= minConfidence) signals.push({ timestamp: last.timestamp, pattern: "BullishEngulfing", confidence: this.clamp(bull) })

      const bear = this.isBearishEngulfing(prev, last)
      if (bear >= minConfidence) signals.push({ timestamp: last.timestamp, pattern: "BearishEngulfing", confidence: this.clamp(bear) })
    }

    const doji = this.isDoji(last)
    if (doji >= minConfidence) signals.push({ timestamp: last.timestamp, pattern: "Doji", confidence: this.clamp(doji) })
  ...
  private deduplicateKeepMax(signals: PatternSignal[]): PatternSignal[] {
    const byKey = new Map<string, PatternSignal>()
